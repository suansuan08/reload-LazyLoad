...
export default class LazyLoad extends Component {
  constructor(props) {
    ...
    this.state = { visible: false, inViewportCount: 0 };
  }
  ...
  lazyLoadHandler() {
    ...
    if (inViewport(node, eventNode, offset)) {
      const { onContentVisible, inViewportCloseLazy } = this.props;
      let inViewportCount = this.state.inViewportCount + 1
      this.setState({ visible: true, inViewportCount: inViewportCount }, () => {
        if (onContentVisible) {
          const isFirstInViewport = inViewportCount > 1 ? false : true;
...
LazyLoad.propTypes = {
  ...
  inViewportCloseLazy: PropTypes.bool,
};
